...
pthread_cond_t condTransferFromSAccount;
pthread_cond_t condTransferFromFAccount;
...
int transfer(transferInfo *arg) {
  ...
  while (arg->from->saldo < valor) {
    ...
    if ((arg->from) == &fAccount) {
      pthread_cond_wait(&condTransferFromFAccount, &mutexTransfer);
    } else {
      pthread_cond_wait(&condTransferFromSAccount, &mutexTransfer);
    }
  }
  ...
  if ((arg->to) == &sAccount) {
    pthread_cond_broadcast(&condTransferFromSAccount);
  } else {
    pthread_cond_broadcast(&condTransferFromFAccount);
  }
...
int main() {
  ...
  pthread_cond_init(&condTransferFromSAccount, NULL);
  pthread_cond_init(&condTransferFromFAccount, NULL);
  ...
  pthread_cond_destroy(&condTransferFromSAccount);
  pthread_cond_destroy(&condTransferFromFAccount);
  printf("\nTransferências concluídas e memória liberada.\n");
  return 0;
}

//  visualizar execucao no replit:
//  https://replit.com/@SrJaizer/TransferenciaBancaria#main.c
